...
main() {
  case $PROMPT in
    watch)
      ...
      # if [[ $ANIME == "Re:ZERO -Starting Life in Another World-" ]]; then
        # python3 ~/dev/animdl/runner.py stream "$ANIME" -r $((EPNUM+1)) --auto --index 6
      # else
      # python3 ~/dev/animdl/runner.py stream "$ANIME" -r $((EPNUM+1)) --auto --index 1
      # fi
      sh /Users/ivan/ani-cli/ani-cli -a $((EPNUM+1)) "$ANIME"
